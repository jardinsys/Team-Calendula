// sys!serverconfig / sys!serversettings - Server configuration (admin only)
// Aliases: sys!serverconfig, sys!servercfg, sys!serversettings
//
// USAGE:
//   sys!serverconfig                           - Show current server settings
//   sys!serverconfig proxy <on|off>            - Enable/disable proxying server-wide
//   sys!serverconfig autoproxy <on|off>        - Allow/force-disable autoproxy
//   sys!serverconfig closedchar <on|off>       - Allow/disallow special characters
//   sys!serverconfig channel blacklist <#channel>
//   sys!serverconfig channel whitelist <#channel>
//   sys!serverconfig channel remove <#channel>
//   sys!serverconfig channel clear
//   sys!serverconfig channel list
...
//   sys!serverconfig admin add <@role|@user>
//   sys!serverconfig admin remove <@role|@user>
//   sys!serverconfig admin list
...
module.exports = {
    name: 'serverconfig',
    aliases: ['servercfg', 'serversettings'],
...
async function handleShow(message, guild) {
    const embed = new EmbedBuilder()
        ...
        .setTitle(`⚙️ ${message.guild.name} - Systemiser Server Config`)
    ...
    embed.setFooter({ text: 'Use sys!serverconfig help for command list' });
...
async function handleProxy(message, parsed, guild) {
    ...
    if (!value || !['on', 'off', 'enable', 'disable'].includes(value)) {
        ...
        return utils.info(message, `Proxying is currently **${current}**.\nUse \`sys!serverconfig proxy on\` or \`sys!serverconfig proxy off\` to change.`);
    }
...
async function handleAutoproxy(message, parsed, guild) {
    ...
    if (!value || !['on', 'off', 'enable', 'disable'].includes(value)) {
        ...
        return utils.info(message, `Autoproxy is currently **${current}**.\nUse \`sys!serverconfig autoproxy on\` to allow or \`sys!serverconfig autoproxy off\` to force disable.`);
    }
...
async function handleClosedChar(message, parsed, guild) {
    ...
    if (!value || !['on', 'off', 'enable', 'disable'].includes(value)) {
        ...
        return utils.info(message, `Special characters are currently **${current}**.\nUse \`sys!serverconfig closedchar on\` to allow or \`sys!serverconfig closedchar off\` to restrict.`);
    }
...
async function handleChannel(message, parsed, guild) {
    ...
    if (action === 'blacklist' || action === 'black' || action === 'bl') {
        ...
        if (guild.channels.whitelist.length > 0)
            return utils.error(message, 'Whitelist mode is active. Use `sys!serverconfig channel clear` first, or use `sys!serverconfig channel remove` to remove from whitelist.');
...
async function handleLog(message, parsed, guild) {
    ...
    // sys!serverconfig log (show current)
    if (!action) {
        const logChannel = guild.channels.logChannel;
        if (!logChannel) return utils.info(message, 'Logging is currently **disabled**.\nUse `sys!serverconfig log #channel` to enable.');
    ...
    // sys!serverconfig log events <event> <on|off>
...
async function handleAdmin(message, parsed, guild) {
    ...
    // sys!serverconfig admin list (or just sys!serverconfig admin)
    if (!action || action === 'list') {
        const embed = new EmbedBuilder()
            .setColor(utils.ENTITY_COLORS.info)
...
/**
 * Show help for serverconfig command
 */
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields(
            ...
            },
            {
                name: '📝 Channels',
                value: [
                    '`sys!serverconfig channel list` - Show channel restrictions',
                    '`sys!serverconfig channel blacklist #channel` - Block proxying in channel',
                    '`sys!serverconfig channel whitelist #channel` - Only allow in channel',
                    '`sys!serverconfig channel remove #channel` - Remove from list',
                    '`sys!serverconfig channel clear` - Clear all restrictions'
                ].join('\n'),
                inline: false
            },
            {
            ...
            },
            {
                name: '👑 Admins',
                value: [
                    '`sys!serverconfig admin list` - List bot admins',
                    '`sys!serverconfig admin add @role|@user` - Add admin',
                    '`sys!serverconfig admin remove @role|@user` - Remove admin'
                ].join('\n'),
                inline: false
            }
        )
